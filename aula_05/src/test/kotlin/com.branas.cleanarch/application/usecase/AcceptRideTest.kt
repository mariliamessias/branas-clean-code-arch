...
class AcceptRideTest : UnitTests() {

    @InjectMockKs
    lateinit var useCase: AcceptRide

    @MockK
    lateinit var repository: RideRepository

    @Test
    fun shouldAcceptRideWithSuccess() {
        val driverId = UUID.randomUUID().toString()
        val rideId = UUID.randomUUID().toString()

        val acceptRideRequest = AcceptRideRequest(
            driverId = driverId,
            rideId = rideId,
        ...
        Assertions.assertDoesNotThrow {
            useCase.execute(acceptRideRequest)
        }
    }
}